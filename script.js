 ...
 document.addEventListener('DOMContentLoaded', function() {
     ...
     document.getElementById('equipmentForm').addEventListener('submit', function(e) {
         ...
         var username = document.getElementById('username').value || 'Desconhecido';
 ...
 function createEquipmentInfo(equipmentItem) {
     ...
     equipmentInfo.innerHTML = `<strong>Equipamento:</strong> ${equipmentItem.equipment}<br>
                                ...
                                <strong>Próxima Manutenção:</strong> ${formatDate(equipmentItem.proximaManutencao)}<br>
                                <div class="update-info">
                                <strong class="last-updated">Última Atualização:</strong> ${formatDateTime(equipmentItem.lastUpdatedAt)}<br>
                                <strong class="updated-by">Atualizado por:</strong> ${equipmentItem.lastUpdatedBy || 'N/A'}
                                </div>
                                `;
...
function updateEquipmentInList(equipmentItem) {
    ...
    if (index !== -1) {
        ...
        equipmentItem.lastUpdatedBy = user; // Substitua pelo nome do usuário real
        equipmentItem.lastUpdatedAt = new Date().toISOString(); // Adiciona data e hora da atualização
    ...
    var oldElement = Array.from(equipmentList.getElementsByClassName('equipment-item')).find(function(el) {
        return JSON.parse(el.getAttribute('data-equipment')).patrimonio === equipmentItem.patrimonio;
    });

    if (oldElement) {
        equipmentList.replaceChild(updatedElement, oldElement);
    } else {
        equipmentList.appendChild(updatedElement); // Caso não encontre, adiciona ao final
    }
...
function shouldHighlight(proximaManutencao) {
 ...
 function logUpdate(equipmentItem) {
    var logs = JSON.parse(localStorage.getItem('updateLogs')) || [];
    var logEntry = {
        dateTime: new Date().toISOString(),
        equipamento: equipmentItem.equipment,
        updatedBy: equipmentItem.updatedBy
    };
    logs.push(logEntry);
    localStorage.setItem('updateLogs', JSON.stringify(logs));
    console.log('Log de atualização registrado:', logEntry);
}

function loadUpdateLogs() {
    var logs = JSON.parse(localStorage.getItem('updateLogs')) || [];
    logs.forEach(function(log) {
        console.log(`Data/Hora: ${log.dateTime}, Equipamento: ${log.equipamento}, Atualizado por: ${log.updatedBy}`);
    });
}
